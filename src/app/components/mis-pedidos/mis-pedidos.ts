...
export class MisPedidos implements OnInit {
  ...
  cargarPedidos(): void {
    ...
    this.http.get(
      `${environment.api.url}${environment.api.endpoints.private.getRentedFromUser}`
    ).subscribe({
      next: (res: any) => {
  ...
  extenderAlquiler(book: any): void {
    ...
    this.http.post(
      `${environment.api.url}${environment.api.endpoints.private.extendRent}`,
      {
        book_id: book.id
      }
    ).subscribe({
  ...
  alquilar(book: any): void {
    ...
    this.http.post(
      `${environment.api.url}${environment.api.endpoints.private.rentBook}`,
      {
        book_id: book.id
      }
    ).subscribe({
