...
export const routes: Routes = [
  ...
  {
    path: 'auth/login',
    loadComponent: () => import('./authentication/log-in/log-in').then((m) => m.LogIn),
  },
  {
    path: 'auth/signup',
    loadComponent: () => import('./authentication/sign-up/sign-up').then((m) => m.SignUp),
  },
  ...
  // CON sidebar (públicas y privadas)
  {
    ...
    children: [
      ...
      // (con authGuard)
      { path: 'carrito', loadComponent: () => import('./carrito/carrito').then((m) => m.Carrito) },
      {
        path: 'perfil',
